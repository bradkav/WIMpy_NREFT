      subroutine CalcWS2(i,j,y,target,WS2)
      implicit none
      real y,WS2
      integer i,j
      character(15) target
...
c     
     ...
     &          + 0.010939268410990407*y**6)/exp(2.*y)
     ...
     &          + 0.011062285090794717*y**6)/exp(2.*y)
         else
     ...
     &          - 0.011000597190320371*y**6)/exp(2.*y)
         end if
      end if
c
...
c
      if (target.eq."N14") then
         if ((i.eq.0).and.(j.eq.0)) then
            WS2 = (0.023007853423118257*(1.2098553307496394 + 1.*y)**2)
     &            /exp(2.*y)
         else
            WS2 = 0.
         end if
...
ccc...
     ...
     -    1.2849348515446646e-7*y**9 + 1.295121171568424e-10*y**10)
     -   /exp(2.*y)
ccc...
       else if ((i.eq.1).and.(j.eq.1)) then
     ...
     -    1.1691210783186218e-7*y**9 + 1.2951223518285722e-10*y**10
     -    )/exp(2.*y)
ccc...
       else
     ...
     -    1.2270279913173943e-7*y**9 - 1.2951217616983635e-10*y**10
     -    )/exp(2.*y)
       end if
...
ccc...
     ...
     -    1.787035062316364e-10*y**10)/exp(2.*y)
ccc...
        else if ((i.eq.1).and.(j.eq.1)) then
     ...
     -    1.7870236298441898e-10*y**10)/exp(2.*y)
ccc...  
        else
     ...
     -    1.787029346070819e-10*y**10)/exp(2.*y)
        end if
...
ccc...
     ...
     -    0.0001895056721698479*y**8)/exp(2.*y)
ccc...
       else if ((i.eq.1).and.(j.eq.1)) then
     ...
     -    0.00018891925256489718*y**8)/exp(2.*y)
ccc...
       else
     ...
     -    0.00018921195056604791*y**8)/exp(2.*y)
       end if
...
ccc...
     ...
     &        + 2.0143142512669966e-9*y**10)/exp(2.*y)
        else if ((i.eq.1).and.(j.eq.1)) then
     ...
     &        + 2.014309360420509e-9*y**10)/exp(2.*y)
        else
     ...
     &        - 2.01431180584053e-9*y**10)/exp(2.*y)
        end if
ccc...        
ccc*F19                                                                                                    
ccc...                                                                                                            
